...
class DownloadViewModel : ViewModel() {
    ...
    private fun calculateDownloadStats(
        ...
        children.forEach { child ->
            val childFile = VideoDownloadManager.getDownloadFileInfo(context, child.id) ?: return@forEach
    ...
    fun updateChildList(context: Context, folder: String) = viewModelScope.launchSafe {
        val visual = withContext(Dispatchers.IO) {
            context.getKeys(folder).mapNotNull { key ->
                ...
                val isSelected = selectedItemIds.value?.contains(it.id) ?: false
                val info = VideoDownloadManager.getDownloadFileInfo(context, it.id) ?: return@mapNotNull null
                ...
                when (which) {
                    DialogInterface.BUTTON_POSITIVE -> {
                        viewModelScope.launchSafe {
                            ...
                            VideoDownloadManager.deleteFilesAndUpdateSettings(context, ids, this) { successfulIds ->
